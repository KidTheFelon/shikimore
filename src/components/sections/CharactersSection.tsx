    ...
    <div className={styles.detailSection}>
      ...
      <HorizontalScroll className={styles.detailCharacters}>
        {characterRoles.slice(0, 30).map((role) => {
          const accentColor = role.character.id
            ? getColor(role.character.id)
            : "transparent";
            ...
            >
              <div className={styles.characterPosterWrapper}>
                {role.character.poster_url ? (
                  <ImageWithSkeleton
                    ...
                    onLoad={
                      role.character.id
                        ? (e) => handleImageLoad(e, role.character.id)
                        : undefined
                    }
                  />
                ) : (
                ...
                {(() => {
                  const allRoles =
                    role.roles_ru && role.roles_ru.length > 0
                      ? role.roles_ru.map(translateRole)
                      : role.roles_en
                        ? (role.roles_en as string[]).map(translateRole)
                        : [];
                  ...
                  return (
                    <div
                      className={`roleBadge ${styles.roleBadge}`}
                      title={allRoles.join(", ")}
                    >
                      <div
                        className={`roleMarqueeContainer ${badgeParams.isLong ? "hasMarquee" : ""}`}
                      >
                        <div
                          className={`roleMarqueeInner ${badgeParams.isLong ? "isMarquee" : ""}`}
                          style={badgeParams.style}
                        >
              ...
              <div className={styles.characterInfo}>
                {(() => {
                  ...
                  return (
                    <div
                      className={`characterNameContainer ${nameParams.isLong ? "hasMarquee" : ""}`}
                    >
                      <div
                        className={`characterMarqueeInner ${nameParams.isLong ? "isMarquee" : ""}`}
                        style={nameParams.style}
                      >
